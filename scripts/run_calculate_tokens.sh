...
TARGET_SCRIPT="$SCRIPT_DIR/calculate_token_counts.py"

# Check if the Python interpreter exists
if [ ! -f "$VENV_PYTHON" ]; then
  echo "Error: Python interpreter not found at $VENV_PYTHON" >&2
  echo "Please ensure the backend virtual environment exists and is located at backend/venv relative to the project root." >&2
  exit 1
fi
